...
public final class App {
    ...
    public static int[] readmx() throws FileNotFoundException {
    ...
    public static double[][] tp(double[][] matrix) {
        double[][] tp = new double[matrix[0].length][matrix.length];
    ...
    public static double determinant(double[][] matrix) {
        ...
        for (int i = 0; i < matrix[0].length; i++)
            det += Math.pow(-1, i) * matrix[0][i]
                * determinant(minormx(matrix, 0, i));
        return det;
    }

    public static double[][] minormx(double[][] matrix, int row, int column) {
        double[][] minormx = new double[matrix.length - 1][matrix.length - 1];

        for (int i = 0; i < matrix.length; i++)
            for (int j = 0; i != row && j < matrix[i].length; j++)
                if (j != column)

                    minormx[i < row ? i : i - 1][j < column ? j : j - 1] = matrix[i][j];
        return minormx;
    }
